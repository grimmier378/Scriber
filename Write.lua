...
local function Output(paramLogLevel, message)
    if Write.loglevels[Write.loglevel:lower()].level <= Write.loglevels[paramLogLevel].level then
        if mq.TLO.MyChatTlo ~= nil then
            mq.TLO.MyChatTlo(Write.prefix,
                string.format('%s%s%s%s%s%s%s%s', type(Write.prefix) == 'function' and Write.prefix() or Write.prefix, GetCallerString(), GetColorStart(paramLogLevel),
                    Write.loglevels[paramLogLevel].abbreviation, GetColorEnd(), type(Write.postfix) == 'function' and Write.postfix() or Write.postfix, Write.separator, message))
        end
        print(string.format('%s%s%s%s%s%s%s%s', type(Write.prefix) == 'function' and Write.prefix() or Write.prefix, GetCallerString(), GetColorStart(paramLogLevel),
            Write.loglevels[paramLogLevel].abbreviation, GetColorEnd(), type(Write.postfix) == 'function' and Write.postfix() or Write.postfix, Write.separator, message))
    end
end
